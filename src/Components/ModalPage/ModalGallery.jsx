            ...
            <div className='relative w-3/6 h-32 bg-white flex justify-between items-center'>
                ...
                <section className='w-6/12 h-full  flex flex-col justify-start items-start p-3'>
                    ...
                    <section className='my-3 w-40 text-xl flex justify-between items-center '>
                        <p className='font-bold text-gray-600'>{price === 'out' ? <p className='text-red-500 text-2xl'>out of stock</p> : <p>${(price - (price * offer / 100)).toFixed(2)}</p>}</p>
                        <p className='text-lg line-through text-rose-500'>{price !== 'out' && <p>${price}</p>} </p>
                    </section>
                        ...
                        <div className='w-6/12 flex justify-start items-center'>
                            {
                                HaveQuantity(id) ?
                                    ...
                                    :
                                    price === 'out' ?
                                        <div className='flex justify-center items-center bg-gray-200 text-gray-800 text-xl font-medium w-48 h-12 cursor-default'> Oops </div> :
                                        <button onClick={AddQuan} className='flex justify-center items-center border-2 border-lime-500 text-lime-600 font-medium w-48 h-12 cursor-pointer hover:bg-lime-500 hover:text-white transition'>Add to cart</button>
                            }
